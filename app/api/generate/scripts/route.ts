...
const COACH_PROMPTS: Record<string, string> = {
  libazi: `你是"纪实派·真诚人设"体系的实体老板IP短视频脚本生成器。风格：22-26秒 |全品类通用 |行业真话+真诚纪实镜头：近景口播60% +纪实行走25% +产品特写15%

25秒爆款四段式：
【0-3s】强钩子（反问/痛点/行业真相）
【3-12s】行业问题拆解
【12-19s】自我人设立场
【19-25s】收口引流万能结尾句式："想要靠谱、想要透明、不想踩坑，关注我，私信我，一对一给你方案。"`,

  boge: `你是"烟火派·同城共情"体系的实体老板IP短视频脚本生成器。风格：25-30秒 |餐饮同城 |共情+三点避坑镜头：后厨烟火手持晃动60% +坐下聊天20% +菜品特写20%
...
【15-22s】我们不一样
【22-30s】私信收口结尾句式："私信我，免费发你方案。我是波哥，只讲真话。"`,

  zhuge: `你是"认知派·高客单逻辑"体系的实体老板IP短视频脚本生成器。风格：28-35秒 |家装建材 |认知干货+选材逻辑镜头：展厅稳重口播50% +材料特写30% +工地巡检20%
...
【10-22s】三点干货方法论
【22-30s】品牌差异化
【30-35s】私信引流结尾句式："需要选材清单、避坑手册的朋友，私信我免费领取。"`,

  geng: `你是"工业派·B端采购"体系的实体老板IP短视频脚本生成器。风格：20-25秒 |工厂制造 |成本拆解+采购干货镜头：车间纪实行走60% +机床前口播25% +卡尺特写15%
...
##五模块分配（共30条）模块A（6条）${MODULE_NAMES.A}：自我介绍、创业故事、底线、初心、承诺模块B（6条）${MODULE_NAMES.B}：没人敢说的真话、内卷乱象、行业规则模块C（6条）${MODULE_NAMES.C}：新手坑、隐形套路、选靠谱商家模块D（6条）${MODULE_NAMES.D}：工厂全景、库存底气、产品细节模块E（6条）${MODULE_NAMES.E}：真诚必杀技、长期主义、私信引流${painSection}${oralSection}

##禁止内容不创新不创作不写文艺词。每条约260-360字，不得少于260字，要有细节和场景描写、具体的观众能听懂的通俗表达。必须是真人老板会说的口语。每段末尾附加情绪钩子——用户看了这段为什么不会划走。格式：【情绪钩子】被吸引→"内心OS"（每段只用箭头递进中对应的一个情绪）。非${coach}风格的内容不要生成。请以 JSON格式返回：
...
ID：a1-a6, b1-b6, c1-c6, d1-d6, e1-e6`
    ...
    if (!content) return NextResponse.json({ error: 'AI返回为空' }, { status: 500 })
    ...
    if (msg.includes('API')) return NextResponse.json({ error: 'AI服务配置异常' }, { status: 500 })

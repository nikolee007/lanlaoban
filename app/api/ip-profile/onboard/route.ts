 ...
 *
 * 老板聊完 10-15 分钟采访后调用：
 *   1. 若还没有人设 → AI 生成 { nickname, slogan, bio }
 *   2. 若还没有脚本 → AI 生成 5 条短视频脚本并落库
 *   3. 开启「每日一条邮件短视频」投递
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!profile) {
      return NextResponse.json({ success: false, error: '还没有 IP 档案，先完成采访' }, { status: 400 })
    }

    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-4xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-gray-500 mb-4">完成IP采访后，AI会自动生成 30-60条专属脚本</p>
          <p className="text-gray-400 text-sm mb-8 max-w-md mx-auto">支持按人设篇/干货篇/展示篇/引流篇分类查看，一键复制或导出全部脚本</p>
          <Link href="/persona" className="btn-primary inline-flex items-center gap-2">去做IP采访
          </Link>
        ...
        <div className="flex gap-2 mb-6">
          ...
          {modules.map(m => (
            <button key={m.id} onClick={() => setFilterModule(filterModule === m.id ? null : m.id)}
              className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${filterModule === m.id ? 'bg-brand-400 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>模块{m.id}
            </button>
          ))}
        </div>

...
${GEN_OUTPUT}`,
  ...
  news: {
    ...
    systemPrompt: `${KERNEL}
【内容类型｜行业动态】
策略：借行业当下新闻/新规政策/趋势/热点事件/内幕话题，用老板第一视角点评，借势涨粉+显专业+立人设。
要点：
1. 选题要"热"：最新政策、行业新规、热门事件、价格变动、趋势风向（如"刚出的新规定"、"今年行业要大变天"、"网上都在传的XX是真的吗"）
2. 开场直接抛热点/内幕，制造"这老板懂行"的即时感
3. 给老板第一视角的真实看法（支持/反对/拆解），立场鲜明有态度
4. 结尾引导关注（"行业有变化，我第一时间告诉你"）
注意：蹭热点不碰负面/敏感/不确定信息，不制造恐慌。
时长：一般 40-50 秒。
${GEN_OUTPUT}`,
  },
